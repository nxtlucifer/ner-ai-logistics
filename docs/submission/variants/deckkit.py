...
FACTS = dict(
    ...
    problem=("NER corridors close without notice: monsoon rain, landslides, floods, official warnings, "
             "single-lane hill roads, no phone signal. A shortest-path router still sends the truck."),
    normal_router=["Origin", "Shortest / fastest road", "Truck"],
    normal_gap=["No terrain, weather, hazard or freshness check.",
                "No governance. No answer when data is missing."],
    ...
    unknown_rule=("Missing or stale evidence is marked UNKNOWN, never counted as safe, "
                  "and the route goes to an authorised reviewer."),
    ...
    clients=("Manager console \u2014 React + TypeScript, manager accounts only \u00b7 Driver app \u2014 React Native / Expo, "
             "Android; one login, server-decided role, offline trip package"),
    ...
    limits=("Render cold start \u2192 warm /health before the demo  \u00b7  provider rate limits \u2192 cached evidence + explicit "
            "UNKNOWN  \u00b7  background push needs Firebase config \u2192 in-app alerts already work  \u00b7  experimental ML \u2192 "
            "outside routing until the gate passes"),
    ...
    ml_recall_sub="on the NER geographic holdout \u2014 154 catalogued events, no NER row used in training",
    ...
    ml_caption=("Strong recall was not enough. Held-out testing exposed the false-positive rate, "
                "so the gate keeps the model out of routing."),
    ml_metrics=("Precision 0.32 \u00b7 F1 0.48 \u00b7 PR-AUC 0.55 \u00b7 ROC-AUC 0.83 \u00b7 Brier 0.12 \u00b7 calibrated logistic "
                "regression on NASA GLC + ERA5-Land rain + DEM slope \u00b7 docs/HAZARD_ERROR_ANALYSIS.md"),
    refs=("gpm.nasa.gov/landslides (NASA GLC)  \u00b7  sachet.ndma.gov.in  \u00b7  global-flood.emergency.copernicus.eu "
          "(GloFAS)  \u00b7  open-meteo.com \u00b7 api.met.no  \u00b7  opentopodata.org \u00b7 Copernicus DEM  \u00b7  project-osrm.org "
          "\u00b7 openstreetmap.org  \u00b7  github.com/nxtlucifer/ner-ai-logistics (repository, tests, model registry)"),
