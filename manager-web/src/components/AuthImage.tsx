...
export default function AuthImage({ src, alt, fallback, className, label }: {
  ...
  const box = className ?? 'h-9 w-9 rounded-full'
  return (
    <span className="inline-flex flex-col items-center gap-0.5" data-testid="auth-image">
      {url ? (
        ...
        <span aria-label={alt} className={`${box} shrink-0 inline-flex items-center justify-center bg-soft text-[11px] font-bold text-muted`}>{fallback}</span>
      )}
